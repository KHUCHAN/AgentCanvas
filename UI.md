...
## 8. 시각 효과 및 애니메이션 (2026-02-20 추가)

### 8.1 글로벌 인터랙션 효과
- **버튼 스케일링:** 모든 버튼 클릭 시 `scale(0.96)` 효과를 적용하여 물리적인 클릭 피드백을 제공합니다.
- **글로우 효과:** 버튼 호버 시 테두리에 미세한 발광(Glow) 효과를 추가하여 주목도를 높였습니다.

### 8.2 화면 전환 애니메이션
- **View Transition:** Kanban, Graph, Schedule 뷰 전환 시 `workspace-body`에 페이드 인 효과를 적용하여 부드러운 전환을 유도합니다.
- **Panel Mode Animation:** RightPanel 내 탭 전환 시 콘텐츠가 오른쪽에서 왼쪽으로 미세하게 슬라이드되며 나타나는 애니메이션을 추가했습니다.

### 8.3 상태 및 진행 피드백
- **Loading Shimmer:** 팀 빌딩이나 태스크 제출 등 시스템이 바쁜(busy) 상태일 때 StatusBar와 주요 버튼에 흐르는 듯한 빛 반사 효과를 적용했습니다.
- **Kanban 카드 강화:**
    - 호버 시 카드 크기 확대 및 그림자 강조.
    - **In Progress:** 실행 중인 태스크 카드에 액센트 컬러 글로우 적용.
    - **Failed:** 실패한 태스크 카드에 미세한 흔들림(Subtle Shake) 효과로 에러 강조.
