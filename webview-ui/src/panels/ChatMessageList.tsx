  ...
  return (
    <div className="chat-messages">
      {props.messages.map((message) => {
        const executionMeta = formatMessageExecutionMeta(message);
        const fallbackBadge = formatFallbackBadge(message);
        return (
          <div key={message.id} className={`chat-message ${message.role}`}>
            {executionMeta && (
              <div className="chat-message-meta">
                <span>{executionMeta}</span>
                {fallbackBadge && <span className="chat-fallback-badge">{fallbackBadge}</span>}
              </div>
            )}
            {message.content.map((content, index) => {
              ...
              if (content.kind === "human_query") {
                return (
                  <div key={key} className="validation-item warning">
                    Human input required ({content.taskId}): {content.question}
                  </div>
                );
              }
...
function formatMessageExecutionMeta(message: ChatMessage): string | undefined {
  const model = message.metadata?.model?.trim();
  if (!model) {
    return undefined;
  }
  const backend = message.metadata?.backendId?.trim();
  if (backend) {
    return `Executed with ${backend} · ${model}`;
  }
  return `Executed with ${model}`;
}

function formatFallbackBadge(message: ChatMessage): string | undefined {
  const fallbackFrom = message.metadata?.fallbackFromModel?.trim();
  const effective = message.metadata?.model?.trim();
  if (!fallbackFrom || !effective) {
    return undefined;
  }
  if (fallbackFrom.toLowerCase() === effective.toLowerCase()) {
    return undefined;
  }
  return `fallback ${fallbackFrom} -> ${effective}`;
}
