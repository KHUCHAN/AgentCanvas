...
### 2.3 실시간 피드백 및 모션
- **상태 변화 애니메이션:** 칸반 카드의 상태 변화(실패 시 흔들림, 실행 중 글로우)를 통해 텍스트를 읽지 않아도 상태 파악이 가능하도록 시각적 힌트를 강화했습니다.
- **인터랙티브 모션:** 버튼 클릭 및 카드 호버 시의 미세한 스케일 변화를 통해 조작감을 향상시키고 사용자에게 즉각적인 반응을 전달합니다.
- **진행 가시성:** 팀 빌드와 같은 긴 작업 중에는 주요 버튼과 상태 바에 Shimmer 효과를 적용하여 시스템이 작동 중임을 명확히 표시합니다.
